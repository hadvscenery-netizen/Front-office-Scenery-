...
(function(){
  ...
  window.__sceneryCloseRoundTableTablePrintFix=true;
  ...
  function buildTable(source){
    ...
    var headers=first.slice(0,19).map(function(cell){return cell.textContent.trim();});
    headers.push('ช่องทางชำระเงิน',first[20].textContent.trim()||'หมายเหตุ');
    ...
    if(!body)body='<tr><td colspan="21">ยังไม่มีรายการที่ Finalized ในวันที่เลือก</td></tr>';
  ...
  function printTable(){
    ...
    if(!table){if(window.showToast)window.showToast('ไม่พบตารางรายละเอียดสำหรับพิมพ์','error');return;}
