...
(function(){
  ...
  function buildSummary(){
    ...
    var labels=['รวม','หักค่าบ้านพักชำระล่วงหน้า','รวมรายได้หน้าฟร้อนวันนี้','เงินสด','บัตรเครดิต','QR Code','โอนเงิน SC','รัฐ 50%','ททท.','ไม่เรียกเก็บ','ค้างชำระ'];
    var lines=[];
    labels.slice(0,3).forEach(function(label,index){
      var item=top[index],value=sourceText(item&&item.querySelector('strong'));
      lines.push('<p>'+esc(label)+': '+esc(value||'-')+'</p>');
    });
    labels.slice(3).forEach(function(label,index){
      var item=paymentRows[index],value=sourceText(item&&item.querySelector('strong'));
      lines.push('<p>'+esc(label)+': '+esc(value||'-')+'</p>');
    });
    return '<section class="print-summary">'+lines.join('')+'</section>';
  }
  function printTable(){
