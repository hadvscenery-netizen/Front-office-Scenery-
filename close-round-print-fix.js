...
(function(){
  ...
  function buildTable(source){
    ...
    headers.push('\u0e23\u0e32\u0e22\u0e44\u0e14\u0e49\u0e2b\u0e19\u0e49\u0e32\u0e1f\u0e23\u0e49\u0e2d\u0e19',first[20].textContent.trim()||'Remark');
  ...
  function sourceText(node){return String(node&&node.textContent||'').trim();}
  function buildSummary(){
    var root=document.querySelector('#view-close-round');
    var top=Array.from((root&&root.querySelectorAll('.close-round-footer-summary>div'))||[]);
    var paymentRows=Array.from((root&&root.querySelectorAll('.payment-bar-row'))||[]);
    var topLabels=['รวม','หักค่าบ้านพักชำระล่วงหน้า','รวมรายได้หน้าฟร้อนวันนี้'];
    var paymentLabels=['เงินสด','บัตรเครดิต','QR Code','โอนเงิน SC','รัฐ 50%','ททท.','ไม่เรียกเก็บ','ค้างชำระ'];
    var topHtml=topLabels.map(function(label,index){
      var item=top[index],value=sourceText(item&&item.querySelector('strong'));
      return '<div class="print-summary-total"><span>'+esc(label)+'</span><strong>'+esc(value||'-')+'</strong></div>';
    }).join('');
    var paymentHtml=paymentLabels.map(function(label,index){
      var item=paymentRows[index],value=sourceText(item&&item.querySelector('strong'));
      return '<div class="print-summary-payment"><span>'+esc(label)+'</span><strong>'+esc(value||'-')+'</strong></div>';
    }).join('');
    return '<section class="print-summary"><div class="print-summary-totals">'+topHtml+'</div><div class="print-summary-payments">'+paymentHtml+'</div></section>';
  }
  function printTable(){
    ...
    var css='@page{size:A4 landscape;margin:0}*{box-sizing:border-box}html,body{width:297mm;min-height:210mm;margin:0;padding:0;background:#fff;color:#211a15;font-family:Arial,Tahoma,sans-serif}.sheet{width:297mm;min-height:210mm;padding:5mm;background:#fff}.heading{display:flex;justify-content:space-between;border-bottom:1.5px solid #6e442d;padding:0 0 1.5mm;margin:0 0 2mm;font-size:16px}.heading span{font-size:11px;color:#66584e}.close-round-print-table{width:287mm;border-collapse:collapse;table-layout:fixed;font-size:11px;line-height:1.12}.close-round-print-table th,.close-round-print-table td{border:1px solid #29231e;padding:1px .7px;vertical-align:middle;overflow-wrap:anywhere;word-break:break-word}.close-round-print-table th{background:#eee3d8;text-align:center;font-weight:700;font-size:8px;line-height:1.05;white-space:normal;overflow-wrap:anywhere;word-break:break-word;vertical-align:middle}.close-round-print-table td.number{text-align:right}.close-round-print-table th:nth-child(1),.close-round-print-table td:nth-child(1){width:9%}.close-round-print-table th:nth-child(2),.close-round-print-table td:nth-child(2){width:6%}.close-round-print-table th:nth-child(3),.close-round-print-table td:nth-child(3){width:15%}.close-round-print-table th:nth-child(4),.close-round-print-table td:nth-child(4),.close-round-print-table th:nth-child(5),.close-round-print-table td:nth-child(5){width:4%}.close-round-print-table th:nth-child(n+6):nth-child(-n+16),.close-round-print-table td:nth-child(n+6):nth-child(-n+16){width:2.8%}.close-round-print-table th:nth-child(n+17):nth-child(-n+19),.close-round-print-table td:nth-child(n+17):nth-child(-n+19){width:4.4%}.close-round-print-table th:nth-child(20),.close-round-print-table td:nth-child(20){width:6%}.close-round-print-table th:nth-child(21),.close-round-print-table td:nth-child(21){width:12%}.close-round-print-table thead{display:table-header-group}.close-round-print-table tbody tr{break-inside:avoid;page-break-inside:avoid}.print-summary{margin-top:4mm;border-top:1.5px solid #6e442d;padding-top:3mm;break-inside:avoid;page-break-inside:avoid}.print-summary-totals{display:grid;grid-template-columns:repeat(3,1fr);gap:2mm;margin-bottom:2mm}.print-summary-total{border:1px solid #cbb9aa;padding:1.5mm;text-align:center}.print-summary-total span{display:block;font-size:9px}.print-summary-total strong{display:block;font-size:11px;margin-top:1mm}.print-summary-payments{display:grid;grid-template-columns:repeat(4,1fr);gap:1.5mm}.print-summary-payment{display:flex;justify-content:space-between;gap:1mm;border:1px solid #cbb9aa;padding:1.5mm;font-size:9px;line-height:1.1}.print-summary-payment strong{font-size:10px}';
    frame.srcdoc='<!doctype html><html><head><meta charset="utf-8"><title>Close Round '+esc(date)+'</title><style>'+css+'</style></head><body><div class="sheet"><div class="heading"><strong>Close Round Details for Accounting</strong><span>Business Date: '+esc(date)+'</span></div>'+table+buildSummary()+'</div></body></html>';
